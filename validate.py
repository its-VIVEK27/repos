...
# Recursive function to get all image paths from nested directories
def get_all_image_paths(root_dir):
    return glob(os.path.join(root_dir, '**', '*.jpg'), recursive=True)  # Modify '*.jpg' if images are in a different format
...
class RealFakeDataset(Dataset):
    ...
    def read_path(self, real_path, fake_path, data_mode, max_sample):
        if data_mode == 'wang2020':
            real_list = [img for img in get_all_image_paths(real_path) if '0_real' in img]
            fake_list = [img for img in get_all_image_paths(fake_path) if '1_fake' in img]
        else:
            real_list = get_all_image_paths(real_path)
            fake_list = get_all_image_paths(fake_path)
...
if __name__ == '__main__':
    ...
    for dataset_path in dataset_paths:
        ...
        print ("AP:", ap)
        print ("Real Accuracy:", r_acc0)
        print ("Fake Accuracy:", f_acc0)
        print ("Accuracy:", acc0)
